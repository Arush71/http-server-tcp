...
func TestRequest(t *testing.T) {
	...
	r, err = RequestFromReader(reader)
	require.NoError(t, err)
	assert.Equal(t, "", r.Headers.Get("Name"))
	...
	r, err = RequestFromReader(reader)
	require.NoError(t, err)
	assert.Equal(t, "localhost:42069, localhost:4539", r.Headers.Get("Host"))
	...
	r, err = RequestFromReader(reader)
	require.Error(t, err)
	assert.Equal(t, r.ParserState, RequestStateParsingHeaders)
}
...
func TestInvalidMethodRequestLine(t *testing.T) {
	// method has lowercase letters → invalid
	_, err := RequestFromReader(strings.NewReader(
		"GeT /coffee HTTP/1.1\r\nHost: localhost:42069\r\n\r\n",
	))
...
func TestBody(t *testing.T) {
	...
	// // Test: Body longer then reported content-length
	// reader = &chunkReader{
	// 	data: "POST /submit HTTP/1.1\r\n" +
	// 		"Host: localhost:42069\r\n" +
	// 		"Content-Length: 0\r\n" +
